<main class="container">

    <section class="main">

        <h2>Formation</h2>

        <div class="cadre row">

            <div class="col-md-6">
                <img class="mediaScreen" src="images/icone2.jpg " height="500px" width="500px" alt="">
            </div>

            <div class="col-md-6">

                <ul class="activite">
                    <li class="point"><h3>» MASTER 2 WEBMARKETING<h3></li>
                    <li>2011 – 2012 // Ecole Supérieure de Management en Alternance, MARNE LA VALLEE</li>
                </ul>

                <ul class="activite">
                    <li class="point"><h3>» MASTER 1 ACTIVITÉS CULTURELLES & TOURISTIQUES<h3></li>
                    <li>2011 // Institut d’Administration des Entreprises (IAE) DIJON</li>
                </ul>

                <ul class="activite">
                    <li class="point"><h3>» DEUG D’ÉCONOMIE ET LICENCE DE GESTION<h3></li>
                    <li>2006 – 2010 // Université de Bourgogne, DIJON</li>
                </ul>

            </div>


        </div>

                        <h2>A propos</h2>

                        <p class="cadre">Besoin d'un coup de pouce en programmation informatique, envie de me connaître un peu plus ou de m'envoyer un message ? Vous trouverez tout le necéssaire ci-dessous. Vous pouvez télécharger ma carte de visite au format vCard, pour m'ajouter facilement à votre carnet d'adresse, m'envoyer un message électronique, me contacter directement par téléphone ou joindre un de mes partenaires. Je m'engage à vous répondre dans la semaine.</p>
     </section>

</main>
